...
def safe_eval_curve(expr_str: str, a: float, b: float, points: int = 1400) -> Tuple[np.ndarray, np.ndarray]:
    """
    Cache-friendly curve evaluation: returns (x_curve, y_curve).
    Keeps cache values pickle-serializable (numpy arrays only).
    """
    expr, f_num = parse_function(expr_str)
    x_curve = build_xcurve(a, b, points)
...
def theory_panel():
    ...
    with st.expander("Open full theory (methods, examples, error & convergence)", expanded=False):
        st.markdown("This engine approximates the definite integral")
        st.latex(r"\displaystyle I = \int_a^b f(x)\,dx")

        st.markdown("by discretizing the interval into **n** subintervals of width")
...
**Example:** \(\int_0^1 x^2 dx = 1/3\). With small n, left rectangles are low because \(x^2\) increases.
""")
...
""")
        ...
        st.markdown("For smooth \(f\):")
        st.latex(r"\displaystyle \text{Error} = O(h^2)")
        st.markdown("Halving \(h\) reduces error by ~4 (quadratic convergence).")
...
""")
        ...
        st.markdown("Smooth case error behaves like:")
        st.latex(r"\displaystyle \text{Error} = -\frac{(b-a)}{12} h^2 f''(\xi) \quad \Rightarrow \quad O(h^2)")
        st.markdown("Stable and widely used in applied settings.")
...
""")
        ...
        st.markdown("Halving \(h\) can reduce error by ~16 for smooth functions.")
...
""")
        ...
        st.markdown("The slope \(p\) is the **observed order of convergence**, estimated numerically by this app.")
...
def make_main_plot(expr_str: str, expr: sp.Expr, f_num, a: float, b: float, n: int, show_rectangles: bool) -> go.Figure:
    h = (b - a) / n
    x_curve, y_curve = safe_eval_curve(expr_str, a, b, points=1400)
...
def make_convergence_plot(expr_str: str, a: float, b: float, method_name: str, n_max: int, step: int, loglog: bool):
